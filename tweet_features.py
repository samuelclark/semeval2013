...
class TweetFeatures:
	def __init__(self,**kargs):
		...
		self.ngram_scores = self.score_ngram_dict(self.ngram_prob) # key : {ngram scores}

		# LENGTH
		self.length_prob = self.get_length_prob() # length {distribution}
		self.length_score = self.score_length_dict(self.length_prob) # key :{length scores}
	...
	def __str__(self):
		res_str = "TweetFeaturesObject:\ninstances: {0}\nmode: {1}\nword: {2}\npos: {3}\n".format(len(self.instances),self.mode,self.word,self.pos)
		return res_str

	def get_tweet_distribution(self):
		# returns the overall distiribution of tweet --> assigned to self.total_distribtuion
	...
	def get_ngram_prob(self):
		...
		# the length of the ngram and pos/word inclusion determine by class settigs
	...
	def get_length_prob(self):
		...
		# the values represent the distribution of polarity for a given phrase length
	...
	def score_context(self):
		...
		for key,parts in self.ct_dict.items():
			context_score_dict = {"objective":0.,"positive":0.,"negative":0.,"neutral":0.}
			target_score_dict = {"objective":0.,"positive":0.,"negative":0.,"neutral":0.}
			context = parts["context"]
			target = parts["target"]
			for ngram in context:
					for label in self.ngram_prob.get(ngram,[]):
						if label!="occurences":
							context_score_dict[label]+=self.ngram_prob[ngram][label]
			for ngram in target:
				for label in self.ngram_prob.get(ngram,[]):
						if label!="occurences":
							target_score_dict[label]+=self.ngram_prob[ngram][label]
