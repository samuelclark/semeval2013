...
class RepeatClassifier(Classifier):
    ...
    """
            Builds a feature for repeating letters such as: heyyy whoaaa etc.
    """
    ...
    def check_repeat_letters(self, word):
    	"""
    		This function executes a regex to find repeating letters in a word
    	"""
        word = word.lower()
    ...
    def get_all_repeats(self):
    	"""
    		This function gathers a comprehensive repeat: frequency dictionary
    	"""
        repeat_dict = {}
    ...
    def build_feature_vector(self, key):
    	"""
    		Builds feature from repeat information
    	"""
        features = {}
        words = [word for word,
        ...
        for repeat in set(self.repeat_dict.keys()):
            features["repeat--{0}({1})".format(self.selection, repeat)
                     ] = (repeat in words)
